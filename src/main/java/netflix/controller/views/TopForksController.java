...
@RestController
public class TopForksController {
    private static Logger logger = LoggerFactory.getLogger(TopForksController.class);

    @Autowired
    private GitServiceAccessor gitServiceAccessor;

    @RequestMapping(
            value = "/view/top/{N}/forks",
    ...
    public ResponseEntity get(@RequestParam(value="N", defaultValue="10") String count) throws IOException {
        ...
        if (repositories != null) {
            List<Repository> orderedRepos = repositories.stream()
                    .collect(Comparators.greatest(N, Comparator.comparingInt(Repository::getForks)));
            List<RepoView> repoViewResult = orderedRepos.stream().map(
                    e -> RepoViewUtil.convertToRepoView(e.getName(), e.getForks()))
